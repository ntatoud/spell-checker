...
int main(int argc, char **argv){
    ...
    if ((NULL == CU_add_test(pSuite, "un caractere avec accent est un caractere alphabetique", test_caractere_alpha_accent))
    || (NULL == CU_add_test(pSuite, "un caractere sans accent est un caractere alphabetique", test_caractere_alpha_sans_accent))
    || (NULL == CU_add_test(pSuite, "un caractere non alphabetique ne l'est pas", test_pas_caractere_alpha))
    || (NULL == CU_add_test(pSuite, "un mot non valide n'est pas valide", test_mot_non_valide)) 
    || (NULL == CU_add_test(pSuite, "un mot valide avec accent est valide", test_mot_valide_avec_accent))
    || (NULL == CU_add_test(pSuite, "un mot valide sans accent est valide", test_mot_valide_sans_accent))
    || (NULL == CU_add_test(pSuite, "creer un mot reduit la casse", test_reduire_la_casse))
    || (NULL == CU_add_test(pSuite, "la copie est identique au mot", test_copier_mot))
    || (NULL == CU_add_test(pSuite, "la copie et le mot sont identiques", test_sont_identiques))
    || (NULL == CU_add_test(pSuite, "ieme caractere renvoie bien le ieme caractere", test_ieme_caractere))
    || (NULL == CU_add_test(pSuite, "supprimer ieme caractere supprimer bien le ieme caractere", test_supprimer_ieme_lettre))
    || (NULL == CU_add_test(pSuite, "inverser deux lettres fontionne", test_inverser_deux_lettres_consecutives))
    || (NULL == CU_add_test(pSuite, "inserer une lettre a la bonne place", test_inserer_lettre))
    || (NULL == CU_add_test(pSuite, "decompo ok", test_decomposer_mot))
